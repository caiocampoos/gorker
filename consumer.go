...
type consumerBuffers map[string]chan *amqp.Delivery
...
func (subs *consumers) buffer(in chan *amqp.Delivery, out chan amqp.Delivery) {
	defer close(out)
	defer subs.Done()
...
func (subs *consumers) add(tag string, consumer chan amqp.Delivery) {
	subs.Lock()
	defer subs.Unlock()
...
func (subs *consumers) send(tag string, msg *amqp.Delivery) bool {
	subs.Lock()
	defer subs.Unlock()
